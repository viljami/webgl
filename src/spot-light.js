...
export default (canvas, compile, bindBuffer) => {
  ...
  void main() {
    ...
    // Compute diffuse point light proportional to this angle
    // vec3 diffuse = lightColor * v_color.rgb * nDotL;

    // Compute diffuse spot light proportional to the spot angle
  ...
  const step = () => {
    ...
    gl.uniform3f(lightPosition, 1.3, 1.3 + Math.cos(angle) * 0.2, 1.3 + Math.sin(angle) * 0.2);
    gl.uniform1f(spotAngle, Math.cos(deg2rad(20)) - Math.cos(angle) * 0.02);
